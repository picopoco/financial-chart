...
def MinMaxScaler(data):
    ...
    denominator = np.max(data, 0) - np.min(data, 0)  #  최대값 - 최소값  최대
...
data = DplyFrame(pd.read_csv('../bitcoin_ticker1.csv', delimiter=','))
#data = data >> sift(X.rpt_key == 'btc_krw')
data = data >> sift(X.rpt_key == 'btc_krw') >> select(X.last, X.diff_24h, X.diff_per_24h, X.bid, X.ask, X.low, X.high, X.volume)

data = np.asarray(data)
#dataminmax = MinMaxScaler(data)
...
test_size = len(dataY) - train_size  # int(len(dataY) * 0.3)
